...
$container_apps_logs = New-AzContainerApp -Name $container_app_name -ResourceGroupName $resource_group_name -Location $location -ConfigurationActiveRevisionsMode 'Single' -ManagedEnvironmentId $container_app_managed_env_id -TemplateContainer $container_app_template
...
if ($?) {
    ...
    "An error occurred while creating $container_app_name"
}
